'''backtest
start: 2018-03-11 23:00:00
end: 2018-04-05 00:00:00
period: 1h
exchanges: [{"eid":"OKCoin_EN","currency":"BTC","balance":100000000,"stocks":3000,"fee":[0.1,0.1]},{"eid":"Bitfinex","currency":"BTC","balance":10000000,"stocks":3000}]
'''
...
ALL_PARAMETERS = {
    ...
    # 平均每单成交数量
    'G_PARAM_BASE_TRADE_AMOUNT': 0.05,
    ...
    # botvs 平台上的测试模式
    'G_PARAM_BOTVS_TEST_MODE': True,
    # 模拟模式的账号余额，实盘模式会忽略此参数
    'G_PARAM_EMU_BALANCE': 1000000,
    # 模拟模式的账号余币，实盘模式会忽略此参数
    'G_PARAM_EMU_STOCKS': 10,
    ...
    # 满足这个套利差百分比再交易，因为1.不能保证交易都成功，不成功只能撤单，丢失成本2.币数量一定，所以应尽可能让每一次交易利润max
    # 使用场景：
    #   - 计算套利交易时，和交易费率一起算作基本交易成本
    # 前一天的套利差百分数
    'G_PARAM_ARBITRAGE_DIFFERENCE': 2,
    ...
    # 交易费率(%), 按买卖顺序添加
    # 若交易所顺序为 okex, huobipro, 交易费率为 0.1, 0.1, 0.2, 0.2
    # 则表示 okex 的买手续费为 0.1%, 卖手续费为 0.1%, huobipro 的分别为 0.2% 和 0.2%
...
DEFAULT_TRDFEE = {'Buy': 0.2, 'Sell': 0.2}

# 有的交易所手续费收取和交易数量有关，有的有时做活动免手续费，因此这里取值为估计均值
TRDFEE_DICT = {
    "binance": {
        "Buy": 0.1,
        "Sell": 0.1
...
class MyExchange(object):
    ...
    def _make_arbit_trade(self, myex_b, slip_point):
        '''
        根据当前买1卖1的挂单计算套利交易
        '''
        a = self
        ...
        result = caculate_arbitrage(self, myex_b, slip_point)
        if not result:
            return None
        ...
        if not a.depth:
            ...
            if a.depth.get('Asks', []):
                asell1_amount = a.depth['Asks'][0]['Amount']  # a交易所中卖1的量
            else:
                asell1_amount = 0.0
            if a.depth.get('Bids', []):
                abuy1_amount = a.depth['Bids'][0]['Amount']  # a交易所中买1的量
            else:
                abuy1_amount = 0.0
        ...
        if not b.depth:
            ...
            if b.depth.get('Asks', []):
                bsell1_amount = b.depth['Asks'][0]['Amount']  # b交易所中卖1的量
            else:
                bsell1_amount = 0.0
            if b.depth.get('Bids', []):
                bbuy1_amount = b.depth['Bids'][0]['Amount']  # b交易所中买1的量
            else:
                bbuy1_amount = 0.0
    ...
    def make_arbit_trade(self, myex_b, slip_point):
        amount_prec = self.amount_prec
        #trade = self._make_arbit_trade(myex_b, slip_point=slip_point)
        trade = self._make_arbit_trade(myex_b, slip_point)
        if not trade:
            return None
        ...
        #每次交易数量
        strategy_amount = get_param('G_PARAM_BASE_TRADE_AMOUNT') 
...
def caculate_price_diff(last_price_diff,
    ...
    '''
    比较两个交易所的差价
    g_real_arbit_diff为参数设置部分的套利差百分数
    ...
    fee_ratio = g_real_arbit_diff / 100.0
...
def caculate_arbitrage(myex_a, myex_b, slip_point):
    '''
    根据当前买1卖1的挂单计算套利交易
    “滑点”暂时设为0，之后根据实际情况可调
    '''
    ...
    a_sell_price = a.get_buy1_price() - slip_point  # 买单价格get_buy1_price()，所以能卖a_sell_price，考虑有滑点，所以真实能卖这么多钱
    ...
    for idx, myex in enumerate(myex_list):  # Account
        if myex.account_refresh_state == 'refresh_instant' or \
                (myex.account_refresh_state == 'refresh_random' and \
                 loop_count % 60 == (60 - idx)):  # 让交易所在不同的循环更新
...
def make_arbit_trades(myex_comp_list):
    '''
    计算应该执行的套利交易单子
    超过2个交易所的时候，可能有好几个套利交易，现根据获取到的信息全部算出来可能的套利交易，再在统一的函数逐个下单
    '''
    result = []
    delay_max = get_param('G_DELAY_MAX')
    slip_point = get_param('G_PARAM_SLIP_POINT')
    for (myex_a, myex_b) in myex_comp_list:
        if not myex_a.is_latest() or not myex_b.is_latest():
            continue
        if myex_a.delay >= delay_max or myex_b.delay >= delay_max:
            continue
        try:
    ...
    for trade in sorted_list:
        ...
        if sell_myex.account['Stocks'] >= get_param('G_PARAM_MIN_TRADE_AMOUNT'):
            ...
            if not buy_order_id and not sell_order_id:
                # 这次套利交易因故不能执行，原因 process_arbitrage_trade 函数已经打印
...
#       - 套利（利差的设定，交易量的设定）
#
# 亏损来源: 套利订单未成交；以法币来衡量价值的话，币值自身的涨跌
# 套利终止原因: 若只有单向套利，模拟回测时买方交易所的资金可能用完了，但实盘时只要及时把钱充进去就不影响
